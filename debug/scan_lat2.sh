...
BASECFG=$SCRIPTS/configs/initLD-trophyV3-3b_muxB_ped.yaml
...
printf '%-8s %-8s %-10s %-10s %-10s %-10s %s\n' \
       "fifo" "L1Aoff" "chip0" "chip1" "chip2" "corrupt" "run"
...
for fifo in $LATENCIES; do
    ...
    out=$(docker exec daq bash -lc "cd $SCRIPTS && \
        PYTHONPATH=\$PWD/analysis python3 pedestal_run.py -d MuxB_lat \
        -i 10.116.24.180 -o $ROOT/Results/alabama -I -f $cfg" 2>&1)
    dir=$(echo "$out" | grep -o "$ROOT/Results/alabama/MuxB_lat/pedestal_run/run_[0-9_]*" | head -1)
    rm -f "$SCRIPTS/$cfg"
...
try:
    ...
    d = t.arrays(['chip','channeltype','adc_stdd','corruption'], library='np')
    m = d['channeltype'] == 0
    out = []
    for c in (0, 1, 2):
        k = m & (d['chip'] == c)
        out.append('%.2f' % np.median(d['adc_stdd'][k]) if k.any() else 'n/a')
    out.append(str(int(d['corruption'][m].sum())))
    print(' '.join(out))
