...
def econAlignment(dev, i2cAddr, linkenablemask = 0xfff, linkinvertmask = 0):
    #set pusm_run
    econI2CWrite(i2cAddr, 0x3c5+0x2, 0x80)
    
    print("FC health")
    print("ECON")
    econI2CWrite(i2cAddr, 0x3a8, 0x01)
    print("FC lock", hex(int.from_bytes(econI2CRead(i2cAddr, 0x3ab).buf[0], "big")))
    print("BCR cnt", hex(int.from_bytes(econI2CRead(i2cAddr, 0x3ad).buf[0], "big")))
    print("BCR cnt", hex(int.from_bytes(econI2CRead(i2cAddr, 0x3ad).buf[0], "big")))

    print("pusm_run", hex(int.from_bytes(econI2CRead(i2cAddr, 0x3c5+0x02).buf[0], "big")))
    print("pusm_status", hex(int.from_bytes(econI2CRead(i2cAddr, 0x3D9+0x6).buf[0], "big")))
        
    # prep econ for alignment
    #econI2CWrite(i2cAddr, 0x3a3, 
    # enable align, disable freeze outputs
    econI2CWrite(i2cAddr, 0x380, 0x02)
    # idle hdr value and mask
    econI2CWrite(i2cAddr, 0x380+0x12, 0x0a)
    #econ reset orbit count max and orbit load cnt
    econI2CWrite(i2cAddr, 0x380+0x13, 0xeb)
    econI2CWrite(i2cAddr, 0x380+0x14, 0x1d)
    econI2CWrite(i2cAddr, 0x380+0x15, 0x00)
    #econ snapshot bx cnt
    econI2CWrite(i2cAddr, 0x380+0x16, 0x15)
    econI2CWrite(i2cAddr, 0x380+0x17, 0x00)
    #econ rx alignment pattern
    econI2CWrite(i2cAddr, 0x380+0x01, 0xaa)
    econI2CWrite(i2cAddr, 0x380+0x02, 0xaa)
    econI2CWrite(i2cAddr, 0x380+0x03, 0xaa)
    econI2CWrite(i2cAddr, 0x380+0x04, 0x9a)
    econI2CWrite(i2cAddr, 0x380+0x05, 0x00)
    econI2CWrite(i2cAddr, 0x380+0x06, 0x00)
    econI2CWrite(i2cAddr, 0x380+0x07, 0x00)
    econI2CWrite(i2cAddr, 0x380+0x08, 0x00)

    # enable/invert links
    linkMask = 1
    for baseaddr in range(0x3e4, 0x3e4+12*0x1, 0x1):
        econI2CWrite(i2cAddr, baseaddr, 0x4 | (0x1 if (linkenablemask & linkMask) else 0x0) | (0x2 if (linkinvertmask & linkMask) else 0x0))
        linkMask = linkMask << 1
    # disable links 6-11
    #for baseaddr in range(0x3e4+6, 0x3e4+12*0x1, 0x1):
    #    econI2CWrite(i2cAddr, baseaddr, 0x4)

    # enable phase training
    for baseaddr in range(0x340, 0x340+12*0x4, 0x4):
        econI2CWrite(i2cAddr, baseaddr, 0x4)

    #for iROC in range(3):
        #invert FC stream and set run bits
        #rocI2CWrite(iROC, 45, 0, 0x23)
    ...
    for baseaddr in range(0x341, 0x341+12*0x4, 0x4):
        print(hex((int.from_bytes(econI2CRead(i2cAddr, baseaddr).buf[0], "big") >> 2)))
    ...
    #prep for snapshot
    #econI2CWrite(i2cAddr, 0x380, 0x06)
    #arm snapshot
    #econI2CWrite(i2cAddr, 0x380, 0x07)
    ...
    #dev.getNode("fastcontrol.request.link_reset_econ").write(1)
    #print("resetroct count:", hex(int.from_bytes(econI2CRead(i2cAddr, 0x3ab+0xa).buf[0], "big")))
    print("resetrocd count:", hex(int.from_bytes(econI2CRead(i2cAddr, 0x3ab+0xb).buf[0], "big")))
    #print("resetecont count:", hex(int.from_bytes(econI2CRead(i2cAddr, 0x3ab+0xc).buf[0], "big")))
    #print("resetecont count:", hex(int.from_bytes(econI2CRead(i2cAddr, 0x3ab+0xd).buf[0], "big")))
    #print(dev.getNode("fastcontrol_recv.counters.link_reset_roct").read())
    #print(dev.getNode("fastcontrol_recv.counters.link_reset_rocd").read())
    #print(dev.getNode("fastcontrol_recv.counters.link_reset_econt").read())
    #print(dev.getNode("fastcontrol_recv.counters.link_reset_econt").read())
...
#   ##########     rocI2CWrite(iROC, 43, 16, 0x67, rocAddresses)
#   ##########     rocI2CWrite(iROC, 43, 17, 0x45, rocAddresses)
#   ##########     rocI2CWrite(iROC, 43, 18, 0x23, rocAddresses)
#   ##########     rocI2CWrite(iROC, 43, 19, 0x01, rocAddresses)
#   ##########     rocI2CWrite(iROC, 89, 16, 0x67, rocAddresses)
#   ##########     rocI2CWrite(iROC, 89, 17, 0x45, rocAddresses)
#   ##########     rocI2CWrite(iROC, 89, 18, 0x23, rocAddresses)
#   ##########     rocI2CWrite(iROC, 89, 19, 0x01, rocAddresses)
    ##########    print(hex(int.from_bytes(getROCStatusReg(iROC, rocAddresses).buf[0], "big")))

    ##########print("\nECOND alignment")
    ##########econAlignment(dev, econdAddress, econdERXEnable, econdERXInvert)
    ##########print("\nECONT alignment")
    ###########invert input links 
    ##########econAlignment(dev, econtAddress, econtERXEnable, econtERXInvert)
    ...
    ##########econI2CWrite(econdAddress, 0x450, 0x5) #FIXME for partial 0x2
    ###########set link processor link rx mask
    ##########econI2CWrite(econdAddress, 0x450+0x1e, 0x3f) #FIXME erx_mask_ebo to 0x7
