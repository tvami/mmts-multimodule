...
class toa_trim_scan_analyzer(analyzer):

    def makePlots(self):
        nchip = len( self.data.groupby('chip').nunique() )        
        cmap = cm.get_cmap('viridis')

        sel_data = self.data[['chip','channel','channeltype','Calib','toa_efficiency','injectedChannels','trim_toa']].copy()
        ...
        for chip in sel_data.groupby('chip')['chip'].mean():
            ...
            for ch in chip_data.groupby('injectedChannels')['injectedChannels'].mean():
                df_chn = chip_data.query('channel==%s & injectedChannels==%s' % (ch,ch)).sort_values('trim_toa')
                if ch<36:
                    ax=axes[0]
                else:
                    ax=axes[1]
                ax.plot(df_chn.trim_toa,df_chn.toa_efficiency,marker='o',color=cmap((ch%36)/36.),label=r'Channel %d'%(ch))

            for half in [0,1]:
                h,l=axes[half].get_legend_handles_labels()
                axes[half].legend(handles=h,labels=l,loc='upper right',ncol=2,fontsize=8)
                
            plt.savefig(f'{self.odir}/trim_toa_scan_chip{chip}.png', format='png', bbox_inches='tight') 
            plt.close()
    ...
    def findTrim(self):
        ...
        for chip in sel_data.groupby('chip')['chip'].mean():
            ...
            trims = {
                ch: {
                    'max0' : chip_data['trim_toa'].min(),
                    'min1' : chip_data['trim_toa'].max() }
                for ch in chip_data.groupby('injectedChannels')['injectedChannels'].mean()
            }
            for ch in chip_data.groupby('injectedChannels')['injectedChannels'].mean():
                df_chn = chip_data.query('channel==%s & injectedChannels==%s' % (ch,ch)).sort_values('trim_toa')
                sel1 = df_chn['toa_efficiency']>0.5
                sel0 = ~sel1
                if sel1.any() and sel0.any():
                    trims[ch]['max0'] = int(df_chn[sel0]["trim_toa"].max())
                    trims[ch]['min1'] = int(df_chn[sel1]["trim_toa"].min())
                    yaml_dict[chip_key_name]['sc']['ch'][ch] = { 'trim_toa' : int( trims[ch]['max0'] + (trims[ch]['min1'] - trims[ch]['max0'])/2 ) }
...
if __name__ == "__main__":
    ...
    ana = toa_trim_scan_analyzer(odir=odir)
