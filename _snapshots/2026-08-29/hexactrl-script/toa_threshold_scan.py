...
def toa_threshold_scan(i2csocket, daqsocket, clisocket, basedir, device_name, injectionConfig, suffix=''):
	...
	###############################################
        for ch in range(18):
                ...
                clisocket.yamlConfig['global']['run_type'] = "toa_threshold_scan"
                ...
                chan = ch
                injectedChannels = [chan,chan+18,chan+36,chan+36+18]
                scan(i2csocket, daqsocket, 150, 350, 2, injectedChannels,calib_dac, rundir,keepRawData=1)
                #scan(i2csocket, daqsocket, 200, 500, 5, injectedChannels,calib_dac, rundir,keepRawData=1)
                clisocket.stop()
        ...
        for ch in range(18):
                ...
                clisocket.yamlConfig['global']['outputDirectory'] = rundir
                clisocket.yamlConfig['global']['run_type'] = "toa_trim_scan"
                clisocket.yamlConfig['global']['serverIP'] = daqsocket.ip
                clisocket.configure()
        ...
        for folder in folders:
                files = glob.glob(folder+"/*.root")
                for f in files[:]:
                        toa_trim_analyzer.add(f)
        toa_trim_analyzer.mergeData()
	...
	###############################################
        ...
        for ch in range(18):
                ...
                injectedChannels = [chan,chan+18,chan+36,chan+36+18]
                scan(i2csocket, daqsocket, 150, 350, 2, injectedChannels, calib_dac, rundir,keepRawData=1)
                #scan(i2csocket, daqsocket, 200, 500, 2, injectedChannels, calib_dac, rundir,keepRawData=1)
                clisocket.stop()
                mylittlenotifier.stop()
...
if __name__ == "__main__" :
        ...
        phase = 14 #i2csocket.yamlConfig['roc_s0']['sc']['Top']['all']['phase_ck']
        ...
        i2csocket.configure()
        toa_threshold_scan(i2csocket, daqsocket, clisocket, options.odir, options.dut, injectionConfig, suffix='')
