...
def vrefinv_scan(i2csocket,daqsocket, clisocket, basedir,device_name,suffix=""):
    ...
    odir = "%s/%s/vrefinv_scan_mars/run_%s/"%( os.path.realpath(basedir), device_name, timestamp ) # a comlete path is needed
    os.makedirs(odir)
    ...
    clisocket.yamlConfig['client']['run_type'] = "vrefinv_scan_mars"
    clisocket.configure()
    ...
    try:
        vrefinv_analyzer = analyzer.vrefinv_scan_analyzer(odir=odir,treename='mars/mars')
...
if __name__ == "__main__":
    ...
    if options.initialize==True:
        ...
        i2csocket.configure()
    vrefinv_scan(i2csocket,daqsocket,clisocket,options.odir,options.dut,suffix="")
